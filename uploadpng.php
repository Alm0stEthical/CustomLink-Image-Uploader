...
$pngCompressionLevel = 9; // PNG compression level from 0 (no compression) to 9 (maximum compression)
    ...
    if ($_POST['login'] === $loginKey) {
        ...
        if ($fileExtension === 'png') {
            ...
            if ($image !== false) {
                ...
                // Apply PNG compression while saving the file
                $compressedPngPath = $uploadPath . '/' . generateRandomString($stringLength) . '.png';
                if (imagepng($image, $compressedPngPath, $pngCompressionLevel)) {
                    imagedestroy($image);
                    echo $websiteUrl . $compressedPngPath;
                } else {
                    die("Error compressing PNG image.");
                }
            } else {
                die("Error loading PNG image.");
            }
        } else {
            // Move the uploaded GIF file to the final path
